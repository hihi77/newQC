...
angular.module('starter.loginControllers', [])
    ...
    .controller('LoginCtrl', function($ionicHistory,DataHand,$interval,$rootScope,$q,$window,$scope,$state,userServices,localStorageService,WeChat){
...
/*                function refreshSysNotify() {
                ...
                function refreshSysNotify() {
                    DataHand.Sdata("get", "refresh_sysNotice", {}, {userId: localStorageService.get('userId')}).then(function (res) {
                        if (res != "") {
                            ...
                            $rootScope.sysNoticeBadge = (!res.allSysNotice||res.allSysNotice<0)?0:res.allSysNotice;
    ...
    .controller('fetchPasswordCtrl', function($ionicPopup,DataHand,$window,$scope,$sce, $timeout,$state, $stateParams,userServices,ShowM,localStorageService) {
        $scope.user = {};
...
//alert(src);
    ...
    .controller('RegisterCtrl', function($ionicPopup,$ionicHistory,$window,$scope,$sce, $timeout,$state, $stateParams,userServices,ShowM,localStorageService) {
            ...
            userServices.findUser({phone:$scope.user.phone.toString()})
                .then(function(res)
                {
                    ...
                    if(res.length<1)
                    {
                        ...
                        $state.go('register_verify',{verify:$scope.user.phone,mobileVerify:$scope.mobileVerify});
                    }
                    else
