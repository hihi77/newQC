angular.module('starter.discoverControllers', [])
  .controller('discover', function ($scope, $rootScope, discoverServices, $ionicPopup, $ionicSlideBoxDelegate) {
    ...
    .controller('discoverSpecialist', function ($state,userServices,$scope, system, $rootScope) {
      //擅长技能 goodSkill
      $scope.title = "擅长技能";
      ...
      system.bindNavTool(
          {
            "rightText": "下一步",
            "toolClick": function () {
              $rootScope.searchSpecilist = $scope.selectData;
              $state.go('tab.discover-addressList');
    ...
    .controller('discoverAddressList', function (discoverServices,$stateParams,$scope, userServices, system, $ionicPopup, $rootScope, $timeout, $ionicModal) {

      $scope.info = {};
      $scope.userFriends = {};
      $scope.allUsers = {};
      discoverServices.searchSpecialist($rootScope.searchSpecilist).then(
          function(result){
    ...
    .controller('discoverSelectType', function ($scope, $rootScope, $ionicHistory, publishActivityServices, system) {
      $scope.title = '选择活动类型';
      $scope.selectTypeText = '选择类型';
      var name = '选择类型';
      if($rootScope&&$rootScope.source=="discover"){
        $scope.maxSurplus = 3;
      }else{
        $scope.maxSurplus = 1;
      }
            ...
            toolClick: function () {
              ...
              $ionicActionSheet.hide = $ionicActionSheet.show({
                ...
                buttonClicked: function (index) {
                  switch (index) {
                    case 0:
                      $stateParams.source = 'user';
                      $state.go('newActivity.edit', $stateParams);
                      break;
                    case 1:
                      $state.go('tab.user-examineAndVerify', {
                        id: $stateParams.id,activityName:stateParam.activityName
                      });
                      break;
                    case 2:
                      $state.go('tab.user-signIn', {
                        id: $stateParams.id
                      });
                      break;
                    case 3:
