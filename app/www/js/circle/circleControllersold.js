...
//使用ios的样式
    ...
    .controller('circleEnterTheCircle', function ($ionicModal,$state,$ionicPopup,$rootScope, $scope, $state, $stateParams, $ionicHistory, $ionicNavBarDelegate, system, circleServices, $timeout, $ionicSlideBoxDelegate) {
      ...
      $scope.entrPublish = function () {
        ...
        /*      if(window.userId&&$rootScope.isMembers){
         $rootScope.publishLayerEs = false;
         $scope.closeCricleLayer = false;
         }else{
         if(!window.userId){
    ...
    .controller('circleManage', function ($ionicModal,$state,$window,$ionicPopup,$scope, $stateParams, circleServices) {
      //圈子权限
      ...
      $scope.leaveCircle = function () {
        ...
        confirmPopup.then(function(res) {
          if(res=='cancle') {
            ...
            circleServices.leaveCircle($stateParams.id).then(function(res){
...
var qrcode = new QRCode(document.getElementById("Qrcode"), {
    text: toUtf8(window.baseUrl.url+'#/tab/circle-addCircleInput/'+$stateParams.id),
    width: 250,
    height: 250,
    colorDark : "#000000",
    colorLight : "#ffffff",
    correctLevel : QRCode.CorrectLevel.H
      ...
      $scope.name = $stateParams.name;
      /*
       $scope.applyCircle = function() {
       $scope.applydata = {}
       // 自定义弹窗
       var myPopup = $ionicPopup.show({
       template: '<input type="text" ng-model="applydata.notice">',
       title: '填写入圈申请',
       subTitle: '',
       scope: $scope,
       buttons: [
       ...
       circleServices.applyCircle($scope.applydata.notice,$scope.id,$scope.circleName);
    ...
    .controller('circleAddAContact', function ($location,  WeChat, $ionicModal, $timeout,$scope, $ionicPopup,$stateParams, circleServices) {
      ...
      $scope.shareWeChatFriend = function () {
        WeChat.sharelink(
            ...
            window.company.logo, window.baseUrl.url+'#/tab/circle-addCircleInput/'+$scope.id
      ...
      $scope.submitSignUp = function () {
        ...
        if (tmp) {
          ...
          circleServices.checkAddUser($scope.userInfo).then(function (res) {
            ...
            circleServices.submitCircleInput($scope.userInfo,$stateParams.id, $scope.registrationAdditional,$scope.data.images).then(function () {
              $ionicPopup.alert({
                "title": "提交成功",
                "okText": "确认"
              })
