...
/*    system.bindNavTool({
  ...
  .controller('jobDescription', function ($scope, $state, $rootScope, $stateParams, $ionicHistory, publishToHireServices, system) {
    ...
    $scope.text = $scope.scope.ephemeralData[a[type]]||'';
    ...
    system.bindNavTool({
      rightText: "保存",
      toolClick: function () {
        $scope.scope.ephemeralData[name] = textarea.val();
        $rootScope.backClick();
      }
    },$scope);
  ...
  .controller('saveInfoOk', function ($state, $scope, $rootScope, $ionicHistory, $stateParams) {
    ...
    $scope.scope.go('tab.discover-index','discover');
  })
  ...
  .controller('selectTmp', function ($stateParams, $scope, $rootScope, $state, $ionicHistory, publishToHireServices, system) {
    ...
    var name = $scope.config.title;
    window.$scope = $scope;
    //console.log($scope.scope.ephemeralData[name])
    if ($scope.scope.ephemeralData[name]) {
      //$scope.selectData = $scope.scope.ephemeralData[name].split(',');
    } else {

    }
    $scope.selectData = $scope.scope.ephemeralData[name] || [];
    system.bindNavTool({
      rightText: "保存",
      toolClick: function () {
        $scope.scope.ephemeralData[name] = [];
        console.log($scope.selectData)
        $scope.selectData.map(function (noe, index) {
          $scope.scope.ephemeralData[name].push(noe.city + '-' + noe.county);
        });
        $rootScope.backClick();
      }
    },$scope);
  ...
  .controller('selectTrade', function ($state, $scope, $rootScope, publishToHireServices, $ionicHistory, system) {
    ...
    system.bindNavTool({
      rightText: "保存",
      toolClick: function () {
        $scope.scope.ephemeralData[name] = $scope.selectData;
        $rootScope.backClick();
      }
    },$scope);
  })
  .controller('technicalAbility', function ($state, $scope, $rootScope, $ionicHistory) {
    ...
    $scope.scope.navRightButtonText = '保存';
    $scope.surplus = 0;
    $scope.selectData = [];
    ...
    $scope.scope.navButtonEvent = function () {
      $scope.scope.ephemeralData[type] = {
  ...
  .controller('yearlySalary', function ($state, $scope, $rootScope, $ionicHistory, publishToHireServices, system) {
    ...
    promise.then(function (result) {
      ...
      if ($scope.scope.ephemeralData[name]) {
        $scope.noeIndex = $scope.yearlySalarys.indexOf($scope.scope.ephemeralData[name]);
      }
    }, function () {
    });
    ...
    system.bindNavTool({
      rightText: "保存",
      toolClick: function () {
        $scope.scope.ephemeralData[name] = $scope.yearlySalarys[$scope.noeIndex];
        $rootScope.backClick();
      }
    },$scope);
  ...
  .controller('toHirePreview', function ($ionicHistory,$scope, $state, $rootScope, publishToHireServices, system, $ionicPopup) {
    ...
    $scope.scope.ephemeralData.postPublisher = window.userInfo;
    //$scope.scope.ephemeralData['岗位特点']=$scope.scope.ephemeralData['岗位特点'].split(',');
    $scope.publishEvent = function () {
      var template = '';
      var showLayer = false;
      if (!$scope.scope.ephemeralData['公司名称']) {
        template += "公司名称未输入<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['地区']) {
        template += "地区未选择<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['职位']) {
        template += "职位未填写<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['行业']) {
        template += "行业未选择<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['职能']) {
        template += "职能未选择<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['年薪']) {
        template += "年薪未选择<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['岗位描述']) {
        template += "岗位描述无内容<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['岗位要求']) {
        template += "岗位要求无内容<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['岗位特点']) {

        template += "岗位特点未选择<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['岗位性质']) {
        template += "岗位性质无内容<br>";
        showLayer = true;
      }
...
/*      if (!isEmail($scope.scope.ephemeralData['公司邮箱'])) {
        template += "公司邮箱格式错误<br>";
        showLayer = true;
      }*/
      if (!($scope.scope.ephemeralData['公司地址'])) {
        template += "公司地址未输入<br>";
        showLayer = true;
      }
      if (isNaN(+$scope.scope.ephemeralData['推荐奖励'])) {
        template += "推荐奖励未填写<br>";
        showLayer = true;
      }
      if (isNaN(+$scope.scope.ephemeralData['推荐成功奖励'])) {
        template += "推荐成功奖励未填写<br>";
        showLayer = true;
      }
      if (!$scope.scope.ephemeralData['选择日期']) {
        template += "日期未选择<br>";
        showLayer = true;
      }
      if (showLayer) {
      ...
      $scope.scope.ephemeralData.createTime=new Date();
      var promise = publishToHireServices.publishInfo(system.newObject($scope.scope.ephemeralData), $stateParams.source, $stateParams.id);
      if (promise) {
        if (promise) {
          promise.success(function () {
            $ionicPopup.show({
            ...
            system.delObj($scope.scope.ephemeralData);
